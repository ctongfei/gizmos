 ...
 */
trait SearchNodeWithCost[+S, C] extends SearchNode[S] {
...
object SearchNodeWithCost {
  ...
  implicit def order[S, C: WeakOrder]: WeakOrder[SearchNodeWithCost[S, C]] = new WeakOrder[SearchNodeWithCost[S, C]] {
    def cmp(x: SearchNodeWithCost[S, C], y: SearchNodeWithCost[S, C]) = x.g >?< y.g
  }

  def apply[S, C](s: S, d: Int, gv: C, p: SearchNodeWithCost[S, C]): SearchNodeWithCost[S, C] = new SearchNodeWithCost[S, C] {
    val state = s
    val depth = d
    val g = gv
    val parent = p
  ...
  def dummy[C]: SearchNodeWithCost[Nothing, C] = new SearchNodeWithCost[Nothing, C] {
