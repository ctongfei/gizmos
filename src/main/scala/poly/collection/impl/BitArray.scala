 ...
 * @author Tongfei Chen
 */
class BitArray(val data: Array[Byte], val length: Int, val offset: Int = 0) extends Iterable[Boolean] {

  import BitArray._

  def apply(bitIdx: Int) = {
  ...
  def newIterator: Iterator[Boolean] = new AbstractIterator[Boolean] {
